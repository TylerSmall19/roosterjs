...
function findHeadLeafNodeInBlock(node: Node, containerBlockNode: Node): Node {
    ...
    while (!blockOrBrEncountered) {
        ...
        while (headNode.previousSibling) {
            ...
            if (isBlockElement(previousSibling) || getTagOfNode(previousSibling) == 'BR') {
        ...
        if (parentPreviousSibiling) {
            if (isBlockElement(parentPreviousSibiling) || getTagOfNode(parentPreviousSibiling) == 'BR') {
                blockOrBrEncountered = true;
                break;
            } else {
...
function findTailLeafNodeInBlock(node: Node, containerBlockNode: Node): Node {
    ...
    while (!blockOrBrEncountered) {
        ...
        while (tailNode.nextSibling) {
            ...
            if (isBlockElement(nextSibling) || (isBr = getTagOfNode(nextSibling) == 'BR')) {
                blockOrBrEncountered = true;
        ...
        if (parentNextSibiling) {
            if (isBlockElement(parentNextSibiling) || (isBr = getTagOfNode(parentNextSibiling) == 'BR')) {
                blockOrBrEncountered = true;
                if (isBr) {
